...
import { create, destroy, edit, index, show } from '@/routes/parking-zones';
...
export default function Index({
    ...
    const filteredParkingZones = useMemo(() => {
        ...
        const filtered = parkingZones.filter((parkingZone) => {
            ...
            const matchesReport =
                reportFilter === 'all' ||
                (reportFilter === 'recent' &&
                    reportIsRecent(parkingZone.last_reported_at)) ||
                (reportFilter === 'stale' &&
                    !!parkingZone.last_reported_at &&
                    !reportIsRecent(parkingZone.last_reported_at)) ||
        ...
        return filtered.sort((a, b) => {
            const occupancyA =
                a.total_spots > 0 ? (a.used_spots / a.total_spots) * 100 : 0;
            const occupancyB =
                b.total_spots > 0 ? (b.used_spots / b.total_spots) * 100 : 0;
    ...
    const recentlyReportedCount = parkingZones.filter((parkingZone) =>
        reportIsRecent(parkingZone.last_reported_at),
    ).length;
    const paidZonesCount = parkingZones.filter(
        (parkingZone) => parkingZone.is_paid,
    ).length;

    const totalAvailableSpots = parkingZones.reduce(
        (sum, parkingZone) => sum + parkingZone.available_spots,
        0,
    );
            ...
            >
                ...
                {filteredParkingZones.length > 0 ? (
                    <Table>
                        ...
                        <TableBody>
                            {filteredParkingZones.map((parkingZone) => {
                                const occupancyRate =
                                    parkingZone.total_spots > 0
                                        ? (parkingZone.used_spots /
                                              parkingZone.total_spots) *
                                          100
                                        : 0;
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <TableCell className="font-medium">
                                            ...
                                            {parkingZone.is_paid && (
                                                <Badge className="ml-2 border-yellow-800 bg-yellow-950 text-xs text-yellow-200">
                                                    Paid
                                                </Badge>
                                            )}
                                        </TableCell>
                                        ...
                                        <TableCell>
                                            <ActionButtons
                                                editUrl={
                                                    edit(parkingZone.id).url
                                                }
                                                destroyUrl={
                                                    destroy(parkingZone.id).url
                                                }
                    ...
                    <NothingFoundList
                        ...
                        title={
                            hasActiveFilters
                                ? 'No parking zones match your filters'
